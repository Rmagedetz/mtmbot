...
class Receipt:
    ...
    def __str__(self):
        if not self.card:
            return ("{}:\nПринял за {} ({}) от {} {} р.\nID:{}".
                    format(self.operator_name, self.receipt_type, self.receipt_comment, self.receipt_issued,
                           self.receipt_sum,
                           self.operation_id))
        else:
            ...
            if username == Lebowski or Users[user_tg_id]["bot_admin"] == 1:
                ...
                try:
                    ...
                    bot.send_message(user_tg_id,
                                     "При попытке отклонения транзакции возникла ошибка. Возможно транзакция отсутствует в таблице\n"
                                     "\nТекст транзакции\n{}".format(text))
            else:
